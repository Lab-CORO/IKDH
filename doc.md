...
$$
F(\mathbf{q}) = FK(\mathbf{q}) - \mathbf{x}_d
\tag{2.1}
$$
...
$$
F(\mathbf{q}_k + \Delta\mathbf{q}) \approx F(\mathbf{q}_k) + J(\mathbf{q}_k)\, \Delta\mathbf{q}
\tag{2.4}
$$
...
$$
\Delta\mathbf{q}_k = -J(\mathbf{q}_k)^{\dagger} F(\mathbf{q}_k)
\tag{2.5}
$$
...
$$
\mathbf{q}_0^{(i)} \in H_N \subset [\mathbf{q}_{\min}, \mathbf{q}_{\max}]^n
\tag{2.7}
$$
...
$$
\mathbf{q}^{(i)} = \text{Newton-Raphson}\left(\mathbf{q}_0^{(i)}\right)
\tag{2.8}
$$
